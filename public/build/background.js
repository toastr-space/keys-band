...
zoo`.split('\n');
    ...
    async function updatePermission(duration, site, domain, type, data) {
    ...
    async function makeResponse(type, data, domain) {
        await loadPrivateKey();
        let res;
        switch (type) {
            case "getPublicKey":
                res = getPublicKey(get_store_value(keyStore));
                break;
            case "getRelays":
            ...
            case "signEvent":
                ...
                res.sig = getSignature(res, get_store_value(keyStore));
                break;
            case "nip04.decrypt":
                try {
                    res = await nip04_exports.decrypt(get_store_value(keyStore), data.peer, data.ciphertext);
                }
                catch (e) {
                    res = {
                        error: {
            ...
            case "nip04.encrypt":
                try {
                    res = await nip04_exports.encrypt(get_store_value(keyStore), data.peer, data.plaintext);
                }
                catch (e) {
                    res = {
                        error: {
    ...
    async function manageRequest(message, sendResponse) {
        return new Promise(async (resolve, reject) => {
            ...
            let resolved = false;
            await loadPrivateKey();
            if (get_store_value(keyStore) === "" || get_store_value(keyStore) === undefined) {
                resolved = true;
                resolve({
                    id: message.id,
                    type: message.type,
            ...
            try {
                site = get_store_value(webSites)[domainToUrl(message.url)];
            }
            catch (e) {
                site = undefined;
            }
            ...
            await new Promise(async () => {
                let options = {
                    action: "popup.html?action=login&url=" +
                        message.url +
                        "&requestId=" +
                        message.id +
                        "&type=" +
